...
<template>
  <div class="min-h-screen bg-pattern p-4 sm:p-6 md:p-8 flex items-center justify-center font-sans" :style="{ backgroundImage: `url(${BgQuestion})` }">
    ...
    <div v-if="isLoading" class="text-center">
      <p class="text-2xl font-bold text-orange-500 animate-pulse">Wong Sabar disayang Tuhan</p>
    </div>
    ...
    <div v-else-if="profileData" class="w-full max-w-6xl mx-auto bg-white rounded-2xl shadow-lg p-4 sm:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
      ...
      <div class="lg:col-span-1 flex flex-col items-center text-center">
        ...
        <div class="mt-6 sm:mt-8 w-full bg-gray-50 p-4 sm:p-6 rounded-lg shadow-inner">
          <h3 class="font-bold text-gray-500 mb-4 text-base sm:text-lg text-center sm:text-center">STATS SINGKAT KAYAK CINTANYA</h3>
          <div class="flex flex-col sm:flex-row justify-around items-center gap-4 sm:gap-0 text-center sm:text-left">
            <div>
              ...
              <p class="text-xs sm:text-sm text-gray-600 text-center sm:text-left">Modul Kelar</p>
            </div>
            <div>
              <p class="text-2xl sm:text-3xl font-bold text-orange-500 text-center sm:text-center">{{ profileData.current_streak }}</p>
              <p class="text-xs sm:text-sm text-gray-600 text-center sm:text-left">Streak Sekarang</p>
            </div>
            <div>
              <p class="text-2xl sm:text-3xl font-bold text-orange-500 text-center sm:text-center">{{ profileData.highest_streak }}</p>
              <p class="text-xs sm:text-sm text-gray-600 text-center sm:text-left">Streak Tertinggi</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2">
        <div class="flex flex-col sm:flex-row justify-between items-center sm:items-start gap-2">
            ...
            <span class="bg-orange-100 text-orange-800 text-xs font-semibold px-2.5 py-0.5 rounded-full mt-2 sm:mt-0 text-center sm:text-left">USER AKTIF</span>
        </div>
        
        <div class="mt-4">
          <p class="text-xs sm:text-sm font-medium text-gray-500 text-center sm:text-left">Progres Belajar (jangan kasih kendor!)</p>
        ...
        <div class="mt-8 sm:mt-10 space-y-6 sm:space-y-8">
            <div>
                <h2 class="text-center sm:text-left text-xl sm:text-2xl font-bold text-gray-700">Modul yang Ditaklukkan</h2>
                <div class="flex flex-col sm:flex-row items-center mt-2 gap-2 sm:gap-4">
                    ...
                    <p class="text-gray-600 text-sm sm:text-base text-center sm:text-left">Dari total semua modul yang ada. Hebat!</p>
                </div>
            </div>
            <div>
                <h2 class="text-center sm:text-left text-xl sm:text-2xl font-bold text-gray-700">Streak Paling Ganas</h2>
                <div class="flex flex-col sm:flex-row items-center mt-2 gap-2 sm:gap-4">
                    ...
                    <p class="text-gray-600 text-sm sm:text-base text-center sm:text-left">Hari belajar berturut-turut. Jangan mau kalah sama mantan yang move on-nya cepet.</p>
                </div>
            </div>
        </div>
      </div>
    ...
    <div v-else class="text-center">
        <p class="text-2xl font-bold text-red-500">Waduh, datanya nyasar! Coba muat ulang.</p>
    </div>
  </div>
</template>